...
export async function POST(req: NextRequest) {
  try {
    ...
    const { name, email, phone, people, date, time, table } = body;
    const isoDate = toStrapiDateTime(date, time);
    if (!isoDate) {
      return NextResponse.json({ error: "Invalid date or time" }, { status: 400 });
    }
    ...
    const msg = {
      ...
      html: `<p>Beste ${name},</p><p>Uw reservering is bevestigd.</p><ul><li><b>Naam:</b> ${name}</li><li><b>Email:</b> ${email}</li><li><b>Telefoon:</b> ${
        phone || "-"
      }</li><li><b>Aantal personen:</b> ${people}</li><li><b>Datum:</b> ${formattedDate}</li><li><b>Tijd:</b> ${formattedTime}</li><li><b>Tafelnummer:</b> ${table}</li><li><b>Locatie:</b> ${location}</li></ul><p><b>Annuleren?</b> Gebruik deze link: <span style='color:#888'>${cancelUrl}</span></p><p>Tot ziens!</p>`,
    };
    try {
